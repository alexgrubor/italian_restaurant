...
<div class="container">
    ...
    <div class="col-md-12">
            ...
            <tbody>
                <?php if ($cart_price->all_price > 0): ?>
                    <?php foreach ($cart_itmes as $cart_item): ?>
                        <tr>
                            <th><img src="../img/<?php echo $cart_item->image; ?>" style="width: 50px; height: 50px"></th>
                            <td>
        ...
        <div class="position-relative mx-auto" style="max-width: 400px; padding-left: 679px;">
            <?php if ($cart_price->all_price > 0): ?>
                <p style="margin-left: -7px;" class="w-40 py-3 ps-4 d-flex pe-5" type="text"> Total: €
                    <?php echo $cart_price->all_price; ?>
                </p>
            <?php endif; ?>
            <form method="POST" action="cart.php">
                <button name="submit" type="submit" class="btn btn-primary py-2 top-0 end-0 mt-2 me-2" <?php echo ($cart_price->all_price == 0) ? 'disabled' : ''; ?>>Checkout</button>
            </form>
        </div>
    </div>
</div>
